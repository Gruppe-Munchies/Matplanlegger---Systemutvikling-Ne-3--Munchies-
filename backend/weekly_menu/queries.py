...
def fetch_weeklymenu_recipes_where_name_usergroupid():
    session = loadSession()
    return session.query(Recipe.name, RecipeHasWeeklyMenu.expectedConsumption,
                         RecipeHasWeeklyMenu.recipe_idRecipe).join(RecipeHasWeeklyMenu,
                                                                   Recipe.idRecipe == RecipeHasWeeklyMenu.recipe_idRecipe).all()


def remove_from_RecipeHasWeeklyMenu(recipeID):
    session = loadSession()
    session.query(RecipeHasWeeklyMenu).filter(RecipeHasWeeklyMenu.recipe_idRecipe == recipeID).delete(
